...
const Orders = () => {
 ...
 interface Order {
  ...
  fulfillmentStatus: "Shipped" | "Processing" | "Cancelled" | "Delivered" | "Unfulfilled" | "Returned";
  date: string;
}
...
const statusStyles = {
 ...
 const columns: ColumnDef<Order>[] = [
  ...
  },
  {
    ...
    header: "FULFILLMENT",
    cell: ({ row }) => {
      const status = row.getValue("fulfillmentStatus") as keyof typeof statusStyles;
      return (
        <span
          className={`inline-block text-xs font-medium px-3 py-1 rounded-full ${statusStyles[status] || "bg-gray-500/10 text-gray-500"}`}
        >
...
const ordersData: Order[]  = [
  ...
  return (
    <div className="px-4 py-6 md:px-8 md:py-8 space-y-6">
      ...
      <div className="rounded-2xl border border-border bg-card/40 backdrop-blur-sm p-4">
        ...
        <div className="hidden xl:flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-3">
            <button className="apply-btn h-11 px-6">
              Apply
            </button>

            <button className="reset-btn h-11 px-6">
              Reset
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 xl:hidden">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button className="apply-btn h-11 w-full">
              Apply
            </button>

            <button className="reset-btn h-11 w-full">
              Reset
            </button>
          </div>
        </div>
      ...
      <div className="rounded-xl overflow-hidden border-b border-x">
